import { Fornecedor } from './classes.js'
...
function carregarLista(divLista) {
    ...
    `
    fetch('/painel/fornecedores/lista?filter=all')
    .then((response) => {
        if (!response.ok) {
            divLista.innerHTML = ''
...
function atualizarLista(divLista) {
    ...
    fornecedores.forEach(element => {
        ...
        let relation = element.relation_level
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="visualizar-item">
                        <p class="label-item">${texts.relation}:</p>
                        <p class="conteudo-item">${relation}</p>
                    </div>
                </div>
        ...
        editarBt.addEventListener('click', () => {
            ...
            editBt.addEventListener('click', () => {
                ...
                let relationValue = document.querySelector('#relation-edit'+ id)
                let editModalBody = document.querySelector('#editModalBody'+id)

                if (nomeValue.value.length < 1) {
                    nomeValue.focus()
                    if (lang == 'es' || lang == 'es-ar') {
                        alertGenerate(editModalBody, 'Nombre Inválido')
                    } else if (lang == 'en') {
                        alertGenerate(editModalBody, 'Invalid Name')
                    } else {
                        alertGenerate(editModalBody, 'Nome Inválido')
                    }
                    editBt.innerHTML = texts.confirmarBt
                    return
                } else if (!relationValue.value || relationValue.value < 1 || relationValue.value > 3) {
                    nomeValue.focus()
                    if (lang == 'es' || lang == 'es-ar') {
                        alertGenerate(editModalBody, 'Seleccione un nivel de relación entre 1 y 3.')
                    } else if (lang == 'en') {
                        alertGenerate(editModalBody, 'Select a relationship level between 1 and 3.')
                    } else {
                        alertGenerate(editModalBody, 'Selecione um nível de relacionamento entre 1 e 3.')
                    }
                    editBt.innerHTML = texts.confirmarBt
                    return
                }
                ...
                send.append('relation', relationValue.value)
                fetch('/painel/fornecedores/lista', {
                    method: 'POST',
                    body: send
                })
                .then((response) => {
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body" id="editModalBody${id}">
                    ...
                    <div class="visualizar-item">
                        <p class="label-item">${texts.relation}:</p>
                        <input required maxlength="50" type="text" class="input-edit" name="email-edit" id="relation-edit${element.id}" value="${relation}">
                    </div>
                    <div class="modal-footer">
        ...
        excluirBt.addEventListener('click', () => {
            ...
            excluirConfirmarBt.addEventListener('click', () => {
                fetch(`/painel/fornecedores/lista?filter=excluir&arg=${id}`)
                .then((response) => {
                    if (!response.ok) {
...
export var novoFornecedor = function() {
    ...
    fetch('/painel/fornecedores/novo?filter=texts')
    .then((response) => {
        if (!response.ok) {
            ...
            .then((data) => {
                ...
                divRelation.className = 'input-item'
                let relationLabel = document.createElement('p')
                relationLabel.className = 'input-label'
                relationLabel.innerHTML = data.relation
                divRelation.appendChild(relationLabel)
                let relationSelect = document.createElement('select')
                relationSelect.className = 'input-input'
                relationSelect.id = 'relationSelect'
                let optDefault = document.createElement('option')
                optDefault.innerHTML = data.option
                optDefault.value = '-1'
                relationSelect.appendChild(optDefault)
                let opt1 = document.createElement('option')
                opt1.innerHTML = 1
                opt1.value = '1'
                relationSelect.appendChild(opt1)
                let opt2 = document.createElement('option')
                opt2.innerHTML = 2
                opt2.value = '2'
                relationSelect.appendChild(opt2)
                let opt3 = document.createElement('option')
                opt3.innerHTML = 3
                opt3.value = '3'
                relationSelect.appendChild(opt3)
                divRelation.appendChild(relationSelect)
                formGroup.appendChild(divRelation)
                ...
                confirmarBt.addEventListener('click', () => {
                    if (inputNome.value.length < 1) {
                        ...
                        return
                    } else if (relationSelect.options[relationSelect.selectedIndex].value == -1) {
                        if (lang == 'es' || lang == 'es-ar') {
                            alertGenerate(body, 'Seleccionar un nivel de relación')
                        } else if (lang == 'en') {
                            alertGenerate(body, 'Select a relationship level')
                        } else {
                            alertGenerate(body, 'Selecione um nível de relacionamento')
                        }
                        return
                    }
                    let send = new FormData()
                    ...
                    send.append('relation', relationSelect.options[relationSelect.selectedIndex].value)
                    fetch('/painel/fornecedores/novo', {
                        method: 'POST',
                        body: send
                    })
                    .then((response) => {
