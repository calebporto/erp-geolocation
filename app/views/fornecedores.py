...
from app.providers.texts import fornecedores_menu_texts, main_en_texts, main_es_texts, main_pt_texts, novo_fornecedor_texts
from app import db
...
def novo():
    ...
    if request.method == 'POST':
        try:
            ...
            if not nome or not relation_level:
                ...
                if lang == 'es' or lang == 'es-ar':
                    messages.append('Proveedor registrado correctamente.')
                elif lang == 'en':
                    messages.append('Successfully registered provider.')
                else:
                    messages.append('Fornecedor registrado com sucesso.')
                response  = Register_Response_(
                    status=True,
                    message=messages
                )
...
@fornecedores_bp.route('/lista', methods=['GET', 'POST'])
def lista():
    lang = request.cookies.get('lang')
    if request.method == 'POST':
        ...
        tel1 = request.form.get('tel1') if request.form.get('tel1') else None
        tel2 = request.form.get('tel2') if request.form.get('tel2') else None
        relation = int(request.form.get('relation')) if int(request.form.get('relation')) else None
        messages = []
        if not id or not nome or not relation:
            messages.append('Campos obrigatórios estão faltando.')
            response  = Register_Response_(
                status=False,
                message=messages
            ...
            provider.tel1 = tel1
            provider.tel2 = tel2
            provider.relation_level = relation
        ...
        filter = request.args.get('filter')
        if filter == 'all':
            provider_list = Person.query.filter(Person.person_type == 1).all()
